  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      if (res?.success && res?.payload) {
        setForm({
          name: res.payload.name || res.payload.firstName ? `${res.payload.firstName || ''} ${res.payload.lastName || ''}`.trim() : 'Admin User',
          email: res.payload.email || '',
          role: res.payload.role || 'Admin',
          employeeId: res.payload.employeeId || res.payload.referenceId || '',
        })
        setLoading(false)
      } else {
        setLoading(false)
