...
function AnalyticsSection({ darkMode }: { darkMode: boolean }) {
  ...
  const diseasesData = (stats?.diseases ?? []).map((r, i) => ({
    ...
    color: COLORS[i % COLORS.length],
    count: r.count,
    pct: parseFloat(((r.count / (diseaseCases || 1)) * 100).toFixed(1)),
  }))
  ...
  const summary = [
    ...
    {
      label: 'Walk-ins Registered',
      value: fmt(walkIns),
      sub: `${(walkInRate * 100).toFixed(1)}% of total`,
      color: '#16A34A',
      svg: (
        <>
    ...
    {
      label: 'Resident Appointments',
      value: fmt(total - walkIns),
      sub: `${((1 - walkInRate) * 100).toFixed(1)}% of total`,
      color: '#F59E0B',
      svg: (
        <>
    ...
    <div className="mb-7">
      ...
      {loading ? (
        ...
        >
          ...
          <span className="text-base font-semibold">Loading analytics data...</span>
        </div>
      ) : (
        <>
          {total === 0 && (
            <div
              className={`p-4 rounded-xl mb-[22px] border ${
                darkMode
            ...
            >
              <p className="m-0 text-sm font-medium">
                No appointments found for <strong>{stats?.rangeLabel ?? rangeKey}</strong>. Try selecting <strong>"All Time"</strong> to view historical appointment analytics.
              </p>
            </div>
          )}
            ...
            {summary.map((s) => (
              <div
                ...
                className={`flex items-center gap-4 max-sm:gap-3 ${
                  darkMode
                    ? 'bg-[#2d1b4e] border-[rgba(255,255,255,0.10)] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.3)]'
                    : 'bg-white border-[rgba(15,60,95,0.10)] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)]'
                } p-[22px] max-sm:p-4 rounded-[18px] border`}
              >
                <div
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-[22px] mb-[22px]">
            ...
            >
              ...
              >
                Percentage of Patients per Service
              </h3>
              <p
                className={`m-0 mb-4 text-[14px] ${
                  darkMode ? 'text-gray-400' : 'text-gray-500'
                }`}
              >
                Share of patients who chose each service in the {rangeLabel}
              </p>
                ...
                {serviceShareData.length > 0 ? (
                  serviceShareData.map((s) => (
                    <div key={s.label} className="flex items-center gap-3">
                      ...
                      />
                      <span
                        className={`w-44 flex-shrink-0 text-[15px] font-semibold truncate ${
                          darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'
                        }`}
                      >
                        {s.label}
                      ...
                      <span
                        className={`w-32 flex-shrink-0 text-right text-[15px] whitespace-nowrap ${
                          darkMode ? 'text-gray-300' : 'text-gray-600'
                        }`}
                      >
                        {s.pct}% &middot; {s.count}
                      </span>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-gray-500 my-4">No service data available.</p>
                )}
              </div>
            </div>
            ...
            >
              ...
              >
                Most common reasons patients scheduled their appointments in the {rangeLabel}
              </p>
              <div className="flex flex-col gap-3">
                {appointmentReasonsData.length > 0 ? (
                  appointmentReasonsData.map((r) => (
                    <div key={r.label} className="flex items-center gap-3">
                      ...
                      />
                      <span
                        className={`w-48 flex-shrink-0 text-[15px] font-semibold truncate ${
                          darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'
                        }`}
                      >
                        {r.label}
                      ...
                      >
                        {r.pct}% &middot; {r.count}
                      </span>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-gray-500 my-4">No appointment reason data available.</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-[22px]">
            ...
            >
              ...
              >
                Status distribution in the {rangeLabel}
              </p>
              {appointmentOutcomesData.length > 0 ? (
                  ...
                  <div className="flex flex-col gap-3">
                    {appointmentOutcomesData.map((o) => (
                      <div key={o.label} className="flex items-center gap-3">
                        ...
                        >
                          {o.pct}% &middot; {o.count.toLocaleString()}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <p className="text-sm text-gray-500 my-4">No appointment outcome data available.</p>
              )}
            </div>
            ...
            >
              ...
              >
                Busiest time slots in the {rangeLabel}
              </p>
              <div className="flex flex-col gap-3">
                {peakHoursData.length > 0 ? (
                  peakHoursData.map((h) => (
                    <div key={h.label} className="flex items-center gap-3">
                      ...
                      />
                      <span
                        className={`w-40 flex-shrink-0 text-[15px] font-semibold whitespace-nowrap ${
                          darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'
                        }`}
                      >
                        {h.label}
                      ...
                      >
                        {h.pct}% &middot; {h.count}
                      </span>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-gray-500 my-4">No peak hour data available.</p>
                )}
              </div>
            </div>
          </div>
          ...
          >
            <div className="flex items-start justify-between flex-wrap gap-2 m-0">
              <div>
                <h3
                  className={`text-xl font-bold m-0 mb-1 ${
                    darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'
                ...
                >
                  Most common diagnosed cases in the {rangeLabel}
                </p>
              </div>
              <span
              ...
              >
                {fmt(diseaseCases)} case{diseaseCases === 1 ? '' : 's'} recorded
              </span>
            </div>
            {diseasesData.length > 0 ? (
              <div className="flex flex-col sm:flex-row items-center justify-center gap-8 mt-5">
                <OutcomesDonut
                  darkMode={darkMode}
                  data={diseasesData}
                  total={diseaseCases}
                ...
                <div className="flex flex-col gap-3">
                  {diseasesData.map((d) => (
                    <div key={d.label} className="flex items-center gap-3">
                      ...
                      >
                        {d.pct}% &middot; {d.count.toLocaleString()}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <p className="text-sm text-gray-500 my-4">
                No medical case data available for the {rangeLabel}.
              </p>
            )}
          </div>
        </>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
        {active ? (
          <>
            ...
            <span
              className={`text-4xl leading-none font-bold mt-1.5 ${
                darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'
              }`}
            >
          ...
          <>
            ...
            <span
              className={`text-sm font-semibold uppercase tracking-[1px] mt-1 ${
                darkMode ? 'text-gray-400' : 'text-gray-500'
              }`}
            >
              {centerLabel}
