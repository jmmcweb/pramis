...
function Card({
  ...
  return (
    <div
      className={`${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} p-[22px] rounded-[18px] border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[rgba(15,60,95,0.10)]'} ${darkMode ? 'shadow-[0_4px_6px_-1px_rgba(0,0,0,0.3)]' : 'shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)]'} flex flex-col gap-[18px]`}
    >
      <div className="flex gap-[18px] items-center">
        <div
          className={`w-[72px] h-[72px] rounded-full flex-shrink-0 flex items-center justify-center text-[28px] bg-[#dedede] text-gray-600`}
        >
          {patient.patientName.charAt(0)}
        </div>
        <div className="flex-1 min-w-0">
          <h3
            className={`text-[24px] font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#111]'} leading-[1] m-0 truncate`}
            title={patient.patientName}
          >
            {patient.patientName}
          </h3>
          <p
            className={`mt-[6px] text-base font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#111]'}`}
          >
            {patient.id}
          </p>
          {patient.patientReference && (
            <p
              className={`mt-[2px] text-[13px] font-semibold ${darkMode ? 'text-[#C4B5FD]' : 'text-[#4E69D3]'}`}
            >
              Ref No: {patient.patientReference}
            </p>
          )}
          <p
            className={`mt-[4px] text-[15px] ${darkMode ? 'text-[#F9FAFB]' : 'text-[#555]'}`}
          >
            {patient.serviceName}
          </p>
        </div>
      ...
      <span
        ...
        className="self-start inline-block px-3 py-1 rounded-full text-[12px] font-bold"
      >
        {patient.serviceName}
      </span>
      <div
        className={`flex flex-col gap-1.5 ${darkMode ? 'bg-[#0f1438]' : 'bg-[#f8fbff]'} px-4 py-[14px] rounded-xl border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[rgba(15,60,95,0.08)]'} shadow-[0_2px_4px_rgba(0,0,0,0.04)] mt-[10px]`}
      >
        <span
          className={`flex items-center gap-2 text-[16px] font-semibold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#111]'}`}
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
      ...
      <div className="flex justify-between gap-[18px]">
        ...
        >
          {patient.hasMedicalRecord ? 'View/Edit Record' : 'Fill Record'}
        </button>
        <button
...
function ListCard({
  ...
  return (
    <div
      className={`${darkMode ? 'bg-[#0f1438]' : 'bg-[#f8fbff]'} border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[rgba(15,60,95,0.08)]'} rounded-xl p-4 flex flex-col gap-2.5`}
    >
      <div className="flex items-center gap-3">
        <div
          className={`w-11 h-11 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-[15px] bg-[#E8EAF6] text-[#4E69D3]`}
        >
          {patient.patientName.charAt(0)}
        </div>
        <div className="flex-1 min-w-0">
          <h4
            className={`text-[16px] font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#111]'} m-0 truncate`}
            title={patient.patientName}
          >
            {patient.patientName}
          </h4>
      ...
      <div className="flex items-center justify-between gap-2">
        <span
          className={`text-[13px] font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#111]'}`}
        >
          {patient.id}
        </span>
        {patient.patientReference && (
          <span
            className={`text-[12px] font-semibold ${darkMode ? 'text-[#C4B5FD]' : 'text-[#4E69D3]'}`}
          >
            Ref No: {patient.patientReference}
          </span>
        )}
      ...
      <div
        className={`flex flex-col gap-1 ${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} px-3 py-2 rounded-lg border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-gray-100'}`}
      >
        <span
          className={`flex items-center gap-2 text-[14px] font-semibold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#111]'}`}
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
...
export default function AppointmentManagementClient({
  ...
  useEffect(() => {
    document.body.style.overflow =
      showArchive || recordFor !== null || showList || showWalkIn
        ? 'hidden'
        : ''
    return () => {
      document.body.style.overflow = ''
    ...
    <div>
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p
                className={`text-[12px] font-semibold m-0 mt-0.5 text-gray-400`}
              >
                General walk-in patients
              </p>
            </div>
            <span
              className={`text-[13px] font-bold px-3 py-1.5 rounded-full ${darkMode ? 'bg-[#0f1438] text-blue-300' : 'bg-blue-500/20 text-blue-600'}`}
            >
              {queues.walkins.filter((q) => q.status !== 'DONE').length} in
              walk-in queue
            </span>
          </div>
          <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto pr-1">
      ...
      {showArchive && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-start z-[1000] p-3 sm:p-4 lg:p-10 overflow-y-auto">
          <div
            className={`${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} rounded-2xl w-full max-w-[960px] shadow-[0_20px_60px_rgba(0,0,0,0.2)] flex flex-col max-h-[90vh]`}
            onClick={(e) => e.stopPropagation()}
          >
            <div
              className={`flex justify-between items-center px-7 py-5 ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-gray-200'} border-b sticky top-0 ${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} rounded-t-2xl z-10`}
            >
              <h2
            ...
            <div
              className={`flex items-center justify-center gap-3 px-7 pt-5 pb-3 ${darkMode ? 'border-b border-[rgba(255,255,255,0.10)]' : 'border-b border-gray-200'}`}
            >
              <button
                onClick={() => {
              ...
              <button
                ...
                className="flex flex-col items-center min-w-[180px] bg-transparent border-none cursor-pointer"
              >
                <span
                  className={`text-base font-bold leading-tight ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'} hover:opacity-70 transition-opacity`}
                >
              ...
              <input
                ...
                className="w-0 h-0 p-0 border-none opacity-0"
              />
              <button
                onClick={() => {
                  const d = new Date(selectedDate + 'T12:00:00')
            ...
            <div className="px-7 py-6 overflow-y-auto flex-1 max-h-[70vh]">
              {archiveForDate.length === 0 ? (
                ...
                >
                  No archived appointments found for this date.
                </p>
              ) : (
                <div className="overflow-x-auto">
                  ...
                  >
                    <thead>
                      ...
                      >
                        <th
                          className={`px-6 py-4 text-left font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'} text-[15px] uppercase tracking-[0.5px] font-poppins border-b w-[34%]`}
                        >
                          Patient Name
                        </th>
                        <th
                          className={`px-6 py-4 text-left font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'} text-[15px] uppercase tracking-[0.5px] font-poppins border-b w-[18%]`}
                        >
                          ID
                        </th>
                        <th
                        ...
                        <th
                          className={`px-6 py-4 text-left font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'} text-[15px] uppercase tracking-[0.5px] font-poppins border-b w-[16%]`}
                        >
                          Time
                        </th>
                        <th
                          className={`px-6 py-4 text-left font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'} text-[15px] uppercase tracking-[0.5px] font-poppins border-b w-[14%]`}
                        >
                          Status
                        </th>
                      </tr>
                    ...
                    <tbody>
                      {archiveForDate.map((p) => {
                        ...
                        return (
                          <tr
                            key={p.id}
                            className={`${darkMode ? 'hover:bg-[#0f1438]' : 'hover:bg-[#E8EAF6]'}`}
                          >
                            <td
                              className={`px-6 py-4 ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[#E2E8F0]'} border-b ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}
                            >
                              <div className="flex items-center gap-3 overflow-hidden">
                                <div
                                  className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 ${darkMode ? 'bg-[#0f1438] text-blue-300' : 'bg-[#E8EAF6] text-[#4E69D3]'}`}
                                >
                            ...
                            <td
                              className={`px-6 py-4 ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[#E2E8F0]'} border-b text-[15px] ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}
                            >
                              {p.id}
                            </td>
                            <td
                              className={`px-6 py-4 ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[#E2E8F0]'} border-b text-[15px] whitespace-nowrap ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}
                            >
                              {p.serviceName}
                            </td>
                            <td
                            ...
                            <td
                              className={`px-6 py-4 ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[#E2E8F0]'} border-b`}
                            >
                              <span
                                style={{ background: st.bg, color: st.color }}
                                className="inline-block px-3 py-1.5 rounded-full text-[12px] font-bold whitespace-nowrap"
                              >
                                {label}
                              </span>
                            </td>
      ...
      {showList && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-start z-[1000] p-3 sm:p-4 lg:p-10 overflow-y-auto">
          <div
            className={`${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} rounded-2xl w-full max-w-[1000px] shadow-[0_20px_60px_rgba(0,0,0,0.2)] flex flex-col max-h-[90vh]`}
            onClick={(e) => e.stopPropagation()}
          >
            <div
              className={`flex justify-between items-center px-7 py-5 ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-gray-200'} border-b sticky top-0 ${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} rounded-t-2xl z-10`}
            >
              <div>
                ...
                <p
                  className={`text-[12px] m-0 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}
                >
                  {listAppointments.length} scheduled appointment(s)
                </p>
              </div>
              <button
                className={`w-9 h-9 border-none ${darkMode ? 'bg-[#0f1438]' : 'bg-gray-100'} rounded-full text-xl ${darkMode ? 'text-[#F9FAFB]' : 'text-gray-500'} cursor-pointer flex items-center justify-center hover:bg-red-500 hover:text-white transition-all`}
            ...
            >
              ...
              >
                <span
                  className={`text-base font-bold leading-tight ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'} hover:opacity-70 transition-opacity`}
                >
                  {new Date(listDate + 'T12:00:00').toLocaleDateString(
                    'en-US',
                    { month: 'long', day: 'numeric', year: 'numeric' },
                  )}
                </span>
            ...
            <div className="px-7 py-6 overflow-y-auto flex-1 max-h-[70vh]">
              {listAppointments.filter((p) => p.dateISO === listDate).length ===
              0 ? (
                <p
                  className={`text-center py-10 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}
                >
                ...
                <div className="grid grid-cols-2 gap-4 max-[768px]:grid-cols-1">
                  {listAppointments
                    .filter((p) => p.dateISO === listDate)
                    .map((p) => (
                      <ListCard
                        key={p.id}
                        patient={p}
                        darkMode={darkMode}
                        onViewRecord={setRecordFor}
                      />
                    ))}
                </div>
              )}
      ...
      {/* Today's Schedule */}
      <div
        className={`${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[rgba(15,60,95,0.08)]'} p-4 rounded-[24px] mb-7 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.06)]`}
      >
        <h2
          className={`text-[26px] sm:text-[32px] lg:text-[40px] ${darkMode ? 'text-[#F9FAFB]' : 'text-[#1d4662]'} m-0 mb-[18px] text-center`}
        >
          Today's Schedule
        </h2>
        {todays.length === 0 ? (
          <p
            className={`text-center py-10 text-[15px] ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}
          >
      ...
      >
        ...
        <p
          className={`text-center text-[15px] m-0 -mt-3 mb-[18px] ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}
        >
          {fmtLong(addDays(now, 1))}
        </p>
      ...
      {showWalkIn && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-start z-[1000] p-3 sm:p-4 lg:p-10 overflow-y-auto">
          <div
            className={`${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} rounded-2xl w-full max-w-[560px] shadow-[0_20px_60px_rgba(0,0,0,0.2)] flex flex-col max-h-[90vh]`}
            onClick={(e) => e.stopPropagation()}
          >
            <div
              className={`flex justify-between items-center px-7 py-5 ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-gray-200'} border-b sticky top-0 ${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} rounded-t-2xl z-10`}
            >
              <div>
                ...
                >
                  New patients get an auto-generated ID — or look up an existing
                  patient
                </p>
              </div>
              <button
            ...
            <div className="px-7 py-6 overflow-y-auto flex-1 flex flex-col gap-5">
              ...
              <div
                className={`flex gap-1 p-1 rounded-xl border ${darkMode ? 'bg-[#0f1438] border-[rgba(255,255,255,0.10)]' : 'bg-[#f8fbff] border-[rgba(15,60,95,0.08)]'}`}
              >
                <button
              ...
              {walkInResult ? (
                  ...
                  {walkInResult.tempPassword && (
                    ...
                    >
                      ...
                      >
                        ...
                        {walkInResult.email ? ` for ${walkInResult.email}` : ''}
                      </p>
                      <p
                      ...
                      >
                        Temporary password:{' '}
                        <span
                          className={`font-mono font-bold text-[15px] ${darkMode ? 'text-[#C4B5FD]' : 'text-[#4E69D3]'}`}
                        >
                          {walkInResult.tempPassword}
                        </span>
                      </p>
                      <p
                        className={`m-0 mt-1 text-[12px] ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}
                      >
                        Give these to the patient — they can sign in and change
                        their password anytime.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-4">
                  <div className="grid grid-cols-2 gap-3 max-[480px]:grid-cols-1">
                    <div>
                      <label htmlFor="wi-first" className={fieldLabel}>
                        First Name *
                      </label>
                      <input
                        id="wi-first"
                    ...
                    <div>
                      <label htmlFor="wi-last" className={fieldLabel}>
                        Last Name *
                      </label>
                      <input
                        id="wi-last"
                    ...
                    <div>
                      <label htmlFor="wi-birth" className={fieldLabel}>
                        Birthday *
                      </label>
                      <input
                        id="wi-birth"
                    ...
                    <div>
                      <label htmlFor="wi-sex" className={fieldLabel}>
                        Sex *
                      </label>
                      <select
                        id="wi-sex"
                    ...
                    <div>
                      <label htmlFor="wi-mobile" className={fieldLabel}>
                        Mobile Number
                      </label>
                      <input
                        id="wi-mobile"
                    ...
                    <div>
                      <label htmlFor="wi-email" className={fieldLabel}>
                        Email (optional)
                      </label>
                      <input
                        id="wi-email"
                    ...
                    <div>
                      <label htmlFor="wi-house" className={fieldLabel}>
                        House / Street
                      </label>
                      <input
                        id="wi-house"
                    ...
                    <div>
                      <label htmlFor="wi-barangay" className={fieldLabel}>
                        Barangay
                      </label>
                      <input
                        id="wi-barangay"
                        value={newInfo.barangay}
                        onChange={setNewField('barangay')}
                        placeholder="Barangay"
                        className={fieldInput}
                      />
                    </div>
                    <div>
                      <label htmlFor="wi-city" className={fieldLabel}>
                        City / Municipality
                      </label>
                      <input
                        id="wi-city"
                    ...
                    <div>
                      <label htmlFor="wi-province" className={fieldLabel}>
                        Province
                      </label>
                      <input
                        id="wi-province"
                        value={newInfo.province}
                        onChange={setNewField('province')}
                        placeholder="Province"
                        className={fieldInput}
                      />
                    </div>
                    <div>
                      <label htmlFor="wi-zip" className={fieldLabel}>
                        Zip Code
                      </label>
                      <input
                        id="wi-zip"
                        value={newInfo.zipCode}
                        onChange={setNewField('zipCode')}
                  ...
                  >
                    A new patient ID (PTN-####) will be generated automatically
                    on save
                ...
                <div>
                  <label
                    htmlFor="walkin-id"
                    className={`block text-[13px] font-bold mb-1.5 ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}
                  >
                    Patient ID *
                  </label>
                  <input
                  ...
                  {isPending && walkInSearch.trim() && (
                    <p
                      className={`m-0 mt-2 text-[13px] font-semibold ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}
                    >
                  ...
                  {!isPending && searchState === 'found' && patient && (
                    <div
                      className={`mt-2.5 rounded-xl border p-3.5 ${darkMode ? 'bg-[#0f1438] border-green-500/30' : 'bg-[#F0FDF4] border-green-200'}`}
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-[15px] bg-[#E8EAF6] text-[#4E69D3]`}
                        >
                          {patient.name.charAt(0)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p
                            className={`m-0 text-[15px] font-bold truncate ${darkMode ? 'text-[#F9FAFB]' : 'text-[#166534]'}`}
                          >
                            {patient.name}
                          </p>
                          <p
                            className={`m-0 text-[13px] font-semibold ${darkMode ? 'text-[#C4B5FD]' : 'text-[#4E69D3]'}`}
                          >
                            {patient.patientId}
                  ...
                  {!isPending && searchState === 'notfound' && (
                    <div className="mt-2">
                      <p
                        className={`m-0 mb-1.5 text-[13px] font-semibold text-amber-500`}
                      >
                      ...
                      >
                        Register as a new patient instead &rarr;
                      </button>
                    </div>
                  )}
                </div>
