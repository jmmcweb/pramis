  ...
  return (
    <div
      className={`flex items-center gap-4 p-5 rounded-[18px] border shadow-[0_4px_6px_-1px_rgba(0,0,0,0.06)] ${
        darkMode ? 'bg-[#2d1b4e] border-[rgba(255,255,255,0.10)]' : 'bg-white border-[rgba(15,60,95,0.08)]'
      }`}
    >
      <div
      ...
      <div className="flex flex-col">
        <span className={`text-4xl max-sm:text-3xl font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>
          {value}
        </span>
  ...
  return (
    <div
      className={`rounded-xl border p-4 ${
        darkMode ? 'bg-[#0f1438] border-[rgba(255,255,255,0.10)]' : 'bg-gray-50 border-gray-200'
      }`}
    >
      <div className="flex flex-wrap justify-between items-center gap-2 mb-3">
        <div>
          <span className={`font-bold text-[16px] font-poppins block ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>
            {formatDate(v.date)}
          </span>
          <span className={`text-[13px] font-poppins ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            {v.serviceName}
          </span>
        </div>
        {v.condition && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-3">
        {[
          ...
          { label: 'O2 Level', value: v.oxygenLevel ? `${v.oxygenLevel}%` : '' },
          ...
          >
            <span className={`block text-[11px] font-semibold uppercase tracking-[0.5px] ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              {m.label}
            </span>
            <span className={`block text-[15px] font-bold font-poppins ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>
              {m.value || '\u2014'}
            </span>
          </div>
        ))}
      ...
      {v.diagnosis && (
        <p className={`m-0 mb-1.5 text-[14px] font-poppins leading-relaxed ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
          <span className={`font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>Diagnosis: </span>
          {v.diagnosis}
        </p>
      )}
      {v.recommendation && (
        <p className={`m-0 mb-1.5 text-[14px] font-poppins leading-relaxed ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
          <span className={`font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>Recommendation: </span>
          {v.recommendation}
        </p>
      )}
      {v.checkedBy && (
        <p className={`m-0 text-[13px] font-poppins italic ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
...
export default function PatientsTable({
  ...
  /** Optional override so staff/midwife shells can pass their own dark-mode state. */
  darkMode?: boolean
}) {
  ...
  const [viewingPatient, setViewingPatient] = useState<PatientListItem | null>(null)
  ...
  const handleSave = () => {
    ...
    if (form.name.trim().length < 3) errors.name = 'Enter the patient\u2019s full name.'
    if (!form.sex) errors.sex = 'Select the patient\u2019s sex.'
    if (!form.birthdate) errors.birthdate = 'Enter a valid birthday (not in the future).'
    ...
    <div>
      ...
      >
          ...
          <div className="relative flex items-center flex-1">
            ...
            <input
              ...
              className={`w-full pl-10 pr-3.5 py-2.5 ${
                ...
                darkMode ? 'text-[#F9FAFB] bg-[#2d1b4e]' : 'text-gray-800 bg-gray-100'
        ...
        <div className="overflow-x-auto rounded-xl">
          <table className="w-full border-collapse text-[16px] min-w-[900px]" style={{ tableLayout: 'fixed' }}>
            <thead>
              <tr className={`${darkMode ? 'bg-[#0f1438]' : 'bg-[#ddd6fe]'}`}>
                {['Patient', 'Age / Sex', 'Contact', 'Address', 'Last Visit'].map((label, i) => (
                  <th
                    key={label}
                ...
                <th className={`px-5 py-4 text-right font-bold ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'} text-[15px] uppercase tracking-[0.5px] font-poppins border-b w-[10%]`}>
                  Actions
                </th>
              </tr>
            </thead>
            <tbody>
              {pagePatients.length === 0 ? (
                <tr>
                  <td colSpan={6} className={`px-5 py-12 text-center text-[16px] font-semibold font-poppins ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    No patients found{q ? ` matching "${searchQuery}"` : ''}.
                  </td>
                </tr>
              ) : (
                pagePatients.map((p) => (
                  <tr key={p.id} className={`border-b last:border-none ${darkMode ? 'border-[rgba(255,255,255,0.08)]' : 'border-gray-100'}`}>
                    <td className="px-5 py-4 align-middle">
                      <div className="flex items-center gap-3 min-w-0">
                        <div
                          className={`w-11 h-11 max-sm:w-9 max-sm:h-9 rounded-full ${
                            darkMode ? 'bg-[#141a45] text-blue-300' : 'bg-[#E8EAF6] text-[#4E69D3]'
                          } flex items-center justify-center font-bold text-sm flex-shrink-0`}
                        >
                          {initials(p.name)}
                        </div>
                        <div className="min-w-0">
                          <p className={`text-[16px] font-bold m-0 truncate font-poppins ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>
                            {p.name}
                          </p>
                          <p className={`text-[13px] font-semibold m-0 truncate font-poppins ${darkMode ? 'text-[#4E9FFF]' : 'text-[#4E69D3]'}`}>
                            {p.id}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className={`px-5 py-4 align-middle text-[15px] font-poppins ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>
                      {p.age ?? '\u2014'} / {p.sex || '\u2014'}
                    </td>
                    <td className={`px-5 py-4 align-middle text-[15px] font-poppins truncate ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                      {p.phoneNumber || '\u2014'}
                    </td>
                    <td className={`px-5 py-4 align-middle text-[15px] font-poppins truncate ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                      {[p.barangay, p.city].filter(Boolean).join(', ') || '\u2014'}
                    </td>
                    <td
                    ...
                    >
                      {p.visits.length > 0 ? (
                        <>
                          <span className="font-bold block truncate">{formatDate(p.visits[0].date)}</span>
                          <span className="text-[13px] block truncate">{p.visits[0].serviceName}</span>
                        </>
                      ) : (
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-5">
          <p className={`m-0 text-[14px] font-poppins ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
      ...
      {showModal && (
        ...
        >
          ...
          >
            <div className={`flex justify-between items-center px-4 sm:px-7 py-4 sm:py-5 ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-gray-200'} border-b`}>
            ...
            <div className="px-4 sm:px-7 py-6 overflow-y-auto flex flex-col gap-3.5">
              <div>
                <label className={`block text-[12px] font-semibold uppercase tracking-[0.5px] mb-1.5 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              ...
              {formError && (
                <p className="m-0 text-[13px] font-semibold text-red-500">{formError}</p>
              )}
            </div>

            <div className={`flex justify-end gap-3 mt-auto px-4 sm:px-7 py-4 border-t ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-gray-200'}`}>
              <button
                onClick={() => !isPending && closeModal()}
                disabled={isPending}
              ...
              >
                {isPending ? 'Saving...' : editingId ? 'Save Changes' : 'Add Patient'}
              </button>
            </div>
          </div>
        </div>
          ...
          >
            <div className={`flex justify-between items-center px-4 sm:px-7 py-4 sm:py-5 ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-gray-200'} border-b`}>
              <div className="flex flex-col">
                ...
                <span className={`text-[13px] font-poppins mt-0.5 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            ...
            <div className="px-4 sm:px-7 py-6 overflow-y-auto flex flex-col gap-4">
              {viewingPatient.visits.length === 0 && (
                <p className={`m-0 text-[15px] font-poppins italic ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  No medical history recorded for this patient yet.
                </p>
              )}
              {viewingPatient.visits.map((v) => (
                <VisitCard key={v.id} v={v} darkMode={darkMode} />
              ))}
            </div>
